...
struct KernelParams {
  ...
  std::string source;
  std::vector<double>
      constants;  // For parameterized kernels (e.g., LeakyReLU alpha)
};

/**
 * @brief Generic GPU kernel executor
 * Provides a unified interface for all activation functions and mathematical
 * operations
 */
...
public:
   ...
   */
  static void executeUnaryKernel(const std::string& kernel_name,
   ...
   */
  static void executeBinaryKernel(const std::string& kernel_name,
...
#ifdef WITH_METAL
  struct MetalKernel {
    void* pipeline;  // id<MTLComputePipelineState> pipeline;
    std::string source;
    size_t param_count;
  };

  static std::unordered_map<std::string, MetalKernel> metal_kernels_;
  static void* device_;         // id<MTLDevice> device_;
  static void* command_queue_;  // id<MTLCommandQueue> command_queue_;

  static void compileMetalKernel(const std::string& name,
                                 const std::string& source, size_t param_count);
  static void executeMetalKernel(
      const std::string& name,
      const std::vector<void*>& buffers,  // std::vector<id<MTLBuffer>>
      size_t size);
#endif
...
public:
   ...
   */
  static void executeActivation(const std::string& name, const double* input,
                                double* output, size_t size,
                                const std::vector<double>& params = {});
...
private:
   ...
   */
  static std::string
  generateKernelSource(const std::string& name, const std::string& expression,
