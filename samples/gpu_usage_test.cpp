...
void testMatrixMultiplication() {
  ...
  for (size_t size : sizes) {
    ...
    try {
      ...
      auto duration_cpu = std::chrono::duration_cast<std::chrono::microseconds>(
          end_cpu - start_cpu);
      printf("⏱️  CPU time: %lld μs (%.2f ms)\n", static_cast<long long>(duration_cpu.count()),
             duration_cpu.count() / 1000.0);
      ...
      try {
        ...
        auto duration_gpu =
            std::chrono::duration_cast<std::chrono::microseconds>(end_gpu -
                                                                  start_gpu);
        printf("⏱️  GPU time: %lld μs (%.2f ms)\n", static_cast<long long>(duration_gpu.count()),
               duration_gpu.count() / 1000.0);
        printf("✅ GPU computation: SUCCESS\n");

        // Calculate speedup
        double speedup = static_cast<double>(duration_cpu.count()) /
            static_cast<double>(duration_gpu.count());
        ...
        if (speedup > 1.2) {
          ...
          printf(
              "🐌 CPU is %.2fx faster than GPU (overhead dominates for this size)\n",
              1.0 / speedup);
        }

        // Performance per element analysis
        double cpu_ns_per_element =
            (duration_cpu.count() * 1000.0) / (size * size * size);
        double gpu_ns_per_element =
            (duration_gpu.count() * 1000.0) / (size * size * size);
        ...
        // Metal uses float precision, so we expect some difference from CPU
        // double precision
        if (max_diff < 1e-6) {
          printf("✅ Excellent precision match (Metal float vs CPU double)\n");
        } else if (max_diff < 1e-3) {
          printf(
              "✅ Good precision (expected for Metal float vs CPU double)\n");
        } else if (max_diff < 1e-1) {
          printf(
              "⚠️  Acceptable precision difference (Metal float limitation)\n");
        } else {
  ...
  printf(
      "- Metal Performance Shaders provide maximum optimization for Apple Silicon\n");
  printf(
      "- Metal uses float32, CPU uses double64 - small precision differences expected\n");
}
...
void testBackendSelection() {
  ...
  for (auto backend : backends) {
    switch (backend) {
    case MLLib::Backend::GPUBackendType::CUDA: printf("  - CUDA\n"); break;
    case MLLib::Backend::GPUBackendType::ROCM: printf("  - ROCm\n"); break;
    case MLLib::Backend::GPUBackendType::METAL: printf("  - Metal\n"); break;
    case MLLib::Backend::GPUBackendType::ONEAPI: printf("  - oneAPI\n"); break;
    default: break;
    }
  }
...
int main() {
  ...
  printf(
      "This test verifies GPU computation effectiveness with large matrices\n\n");
