...
### 2. GPU パフォーマンステスト (`gpu_usage_test.cpp`)

**目的**: 様々なサイズの行列演算でGPU vs CPU のパフォーマンスをベンチマーク。

**機能**:
- 異なるサイズ（256x256から2048x2048）での行列積パフォーマンステスト
- GPUとCPUの実行時間比較
- パフォーマンス速度向上率の計算
- GPUとCPU結果間の数値精度検証
- GPU バックエンド選択と切り替えのテスト
...
**主要パフォーマンス洞察**:
- **小行列 (256x256)**: GPUオーバーヘッドによりCPUの方が高速な場合がある
- **中行列 (512x512)**: パフォーマンスが均衡し始める
- **大行列 (1024x1024以上)**: GPUが大幅な優位性を示す
- **Metalバックエンド**: Apple Silicon向けに最適化、float32精度を使用

### 3. XOR ニューラルネットワーク学習 (`xor.cpp`)

**目的**: XOR問題を用いた完全な機械学習ワークフローの実演。

**機能**:
- Denseレイヤーと活性化関数を使用したニューラルネットワーク構築
- 学習でのGPU加速使用
- 進捗監視のための学習コールバック実装
- 学習中のモデルチェックポイント保存（10エポックごと）
