...
void simulateIntelMacAMDGPU() {
    ...
    std::cout << "  - Architecture: Intel Mac (hw.optional.arm64 = 0)" << std::endl;
    ...
    std::cout << "  - Metal Support: Available (but vendor should still be AMD)" << std::endl;
    std::cout << std::endl;
...
Graphics/Displays:
    ...
    std::cout << "AMD GPU detection: " << (amd_detected ? "✅ DETECTED" : "❌ NOT DETECTED") << std::endl;
    
    // Check Apple Silicon detection (should be false for Intel Mac)
    bool apple_silicon = false; // Simulate hw.optional.arm64 = 0
    std::cout << "Apple Silicon detection: " << (apple_silicon ? "✅ DETECTED" : "❌ NOT DETECTED (Correct for Intel Mac)") << std::endl;
    ...
    std::cout << "Metal API support: " << (metal_support ? "✅ AVAILABLE" : "❌ NOT AVAILABLE") << std::endl;
    ...
    std::cout << "   - Vendor: Apple (incorrect - this is AMD hardware)" << std::endl;
    std::cout << "   - Primary GPU Vendor: Apple" << std::endl;
    std::cout << std::endl;
    std::cout << "🔍 Key Point: Metal API availability does not make it an Apple GPU!" << std::endl;
    std::cout << "   Metal is available on AMD/NVIDIA GPUs on macOS, but the vendor" << std::endl;
    std::cout << "   should reflect the actual hardware manufacturer." << std::endl;
}
...
Graphics/Displays:
    ...
    std::cout << "NVIDIA GPU detection: " << (nvidia_detected ? "✅ DETECTED" : "❌ NOT DETECTED") << std::endl;
    ...
    std::cout << "   - API Support: CUDA (if CUDA installed) or OpenCL/Metal" << std::endl;
    std::cout << "   - Primary GPU Vendor: NVIDIA" << std::endl;
}

int main() {
    std::cout << "=== GPU Vendor Detection Scenario Testing ===" << std::endl;
    std::cout << "This test simulates different Mac configurations to verify" << std::endl;
    ...
    std::cout << "The current MLLib GPU detection logic should correctly:" << std::endl;
    std::cout << "1. Distinguish hardware vendor from API support" << std::endl;
    std::cout << "2. Report AMD vendor for AMD hardware (even with Metal support)" << std::endl;
    std::cout << "3. Report NVIDIA vendor for NVIDIA hardware (even with Metal support)" << std::endl;
    std::cout << "4. Only report Apple vendor for Apple Silicon integrated GPUs" << std::endl;
    std::cout << "5. Correctly detect GPU memory:" << std::endl;
    std::cout << "   - Apple Silicon: Unified memory (75% of system RAM)" << std::endl;
    std::cout << "   - AMD/NVIDIA: Dedicated VRAM from system_profiler" << std::endl;
    std::cout << "   - Intel integrated: Shared memory (25% of system RAM)" << std::endl;
    std::cout << std::endl;
    std::cout << "If you're seeing incorrect vendor detection on Intel Mac + discrete GPU," << std::endl;
    std::cout << "please run the actual device_detection sample and compare with these" << std::endl;
    std::cout << "expected results." << std::endl;
