...
int main() {
  ...
  auto save_callback = [&model](int epoch, double loss) {
    if (epoch % 10 == 0) {
      ...
      try {
        // バイナリ形式で保存
        MLLib::model::ModelIO::save_model(model, model_path + ".bin", MLLib::model::ModelFormat::BINARY);
        // JSON形式でも保存（デバッグ用）
        MLLib::model::ModelIO::save_model(model, model_path + ".json", MLLib::model::ModelFormat::JSON);
        std::cout << "Model saved at epoch " << epoch << " to " << model_path << std::endl;
      } catch (const std::exception& e) {
        std::cerr << "Failed to save model at epoch " << epoch << ": " << e.what() << std::endl;
      }
    }
  };
  ...
  try {
    MLLib::model::ModelIO::save_model(model, final_model_path + ".bin", MLLib::model::ModelFormat::BINARY);
    MLLib::model::ModelIO::save_model(model, final_model_path + ".json", MLLib::model::ModelFormat::JSON);
    MLLib::model::ModelIO::save_model(model, final_model_path + ".config", MLLib::model::ModelFormat::CONFIG);
