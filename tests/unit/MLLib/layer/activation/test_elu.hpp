...
protected:
  void test() override {
    ...
    assertNear(output[0], 1.0 * (std::exp(-2.0) - 1), 1e-9, "ELU for negative input");
    assertNear(output[1], 1.0 * (std::exp(-0.5) - 1), 1e-9, "ELU for negative input");
    ...
    assertNear(grad_input[0], 1.0 * std::exp(-2.0), 1e-9, "ELU gradient for negative input");
    assertNear(grad_input[1], 1.0 * std::exp(-0.5), 1e-9, "ELU gradient for negative input");
