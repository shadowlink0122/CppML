...
protected:
  void test() override {
    ...
    assertTrue(output[2] > 0.5, "GELU for positive input should be positive and substantial");
    
    // GELU should be small for negative inputs
    assertTrue(output[0] < 0.0 && output[0] > -0.5, "GELU for negative input should be small negative");
  }
};
...
protected:
  void test() override {
    ...
    assertTrue(output[2] > 0.5, "Approximate GELU for positive input should be positive and substantial");
    assertTrue(output[0] < 0.0 && output[0] > -0.5, "Approximate GELU for negative input should be small negative");
  }
};
