...
protected:
  void test() override {
    ...
    assertTrue(Device::getCurrentDevice() == DeviceType::CPU,
               "Should be set to CPU");

    // Test device type string conversion
    assertTrue(std::string(Device::getDeviceTypeString(DeviceType::CPU)) ==
                   "CPU",
               "CPU device string should be 'CPU'");
    assertTrue(std::string(Device::getDeviceTypeString(DeviceType::GPU)) ==
                   "GPU",
               "GPU device string should be 'GPU'");
    assertTrue(std::string(Device::getDeviceTypeString(DeviceType::AUTO)) ==
                   "AUTO",
...
protected:
  void test() override {
    ...
    assertNear(result.data()[0], 19.0, 1e-10,
               "Matrix multiplication result [0,0] should be 19");
    assertNear(result.data()[1], 22.0, 1e-10,
               "Matrix multiplication result [0,1] should be 22");
    assertNear(result.data()[2], 43.0, 1e-10,
               "Matrix multiplication result [1,0] should be 43");
    assertNear(result.data()[3], 50.0, 1e-10,
               "Matrix multiplication result [1,1] should be 50");
    ...
    assertNear(result.data()[2], 10.0, 1e-10,
               "Addition result [2] should be 10");
    assertNear(result.data()[3], 12.0, 1e-10,
               "Addition result [3] should be 12");

    // Test element-wise subtraction
    result = a - b;
    assertNear(result.data()[0], -4.0, 1e-10,
               "Subtraction result [0] should be -4");
    assertNear(result.data()[1], -4.0, 1e-10,
               "Subtraction result [1] should be -4");
    assertNear(result.data()[2], -4.0, 1e-10,
               "Subtraction result [2] should be -4");
    assertNear(result.data()[3], -4.0, 1e-10,
               "Subtraction result [3] should be -4");

    // Test element-wise multiplication
    result = a * b;
    assertNear(result.data()[0], 5.0, 1e-10,
               "Multiplication result [0] should be 5");
    assertNear(result.data()[1], 12.0, 1e-10,
               "Multiplication result [1] should be 12");
    assertNear(result.data()[2], 21.0, 1e-10,
               "Multiplication result [2] should be 21");
    assertNear(result.data()[3], 32.0, 1e-10,
               "Multiplication result [3] should be 32");

    // Test scalar operations
    result = a * 2.0;
    assertNear(result.data()[0], 2.0, 1e-10,
               "Scalar multiplication result [0] should be 2");
    assertNear(result.data()[1], 4.0, 1e-10,
               "Scalar multiplication result [1] should be 4");
    assertNear(result.data()[2], 6.0, 1e-10,
               "Scalar multiplication result [2] should be 6");
    assertNear(result.data()[3], 8.0, 1e-10,
               "Scalar multiplication result [3] should be 8");

    result = a + 10.0;
    assertNear(result.data()[0], 11.0, 1e-10,
               "Scalar addition result [0] should be 11");
    assertNear(result.data()[1], 12.0, 1e-10,
               "Scalar addition result [1] should be 12");
    assertNear(result.data()[2], 13.0, 1e-10,
               "Scalar addition result [2] should be 13");
    assertNear(result.data()[3], 14.0, 1e-10,
               "Scalar addition result [3] should be 14");
...
protected:
  void test() override {
    ...
    try {
      // Create model with GPU device (should handle GPU unavailability
      // gracefully)
      ...
      if (Device::isGPUAvailable()) {
        assertTrue(model_device == DeviceType::GPU,
                   "Model should use GPU when available");
      } else {
        assertTrue(model_device == DeviceType::CPU,
                   "Model should fallback to CPU when GPU not available");
      }

      // Test device switching
      model.set_device(DeviceType::CPU);
      assertTrue(model.get_device() == DeviceType::CPU,
                 "Model device should switch to CPU");

      // Try switching back to GPU
      model.set_device(DeviceType::GPU);
      if (Device::isGPUAvailable()) {
        assertTrue(model.get_device() == DeviceType::GPU,
                   "Model should switch to GPU when available");
      } else {
        assertTrue(model.get_device() == DeviceType::CPU,
                   "Model should stay on CPU when GPU not available");
      }
...
protected:
  void test() override {
    ...
    for (int iter = 0; iter < 3; ++iter) {
      ...
      assertEqual(size, result.shape()[0],
                  "Result should have correct dimensions");
      assertEqual(size, result.shape()[1],
                  "Result should have correct dimensions");

      // Check a few specific values for consistency
      double first_value = result.data()[0];
      assertTrue(first_value > 0,
                 "Matrix multiplication should produce positive results");
    }
    ...
    // Check a few values
    assertNear(large_result.data()[0], 1.0, 1e-10,
               "Large array addition [0] should be correct");
    assertNear(large_result.data()[10], 21.0, 1e-10,
               "Large array addition [10] should be correct");
    assertNear(large_result.data()[99], 199.0, 1e-10,
               "Large array addition [99] should be correct");
