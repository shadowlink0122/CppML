...
protected:
  void test() override {
    ...
    assertEqual(any_gpu, !gpus.empty(), "isGPUAvailable() should match detected GPU count");
  }
};
...
protected:
  void test() override {
    ...
    // Verify shapes
    assertTrue(add_result.shape() == a.shape(), "Addition should preserve shape");
    assertTrue(sub_result.shape() == a.shape(), "Subtraction should preserve shape");
    assertTrue(mul_result.shape() == a.shape(), "Multiplication should preserve shape");
    
    // Test scalar operations
    NDArray scalar_result = a * 2.0;
    assertNear(scalar_result.data()[0], 2.0, 1e-10, "Scalar multiplication should work");
    ...
    for (const auto& gpu : gpus) {
      ...
      assertTrue(test_result.size() == a.size(), "Operations should work with all backends");
    }
  ...
  void test() override {
    ...
    if (gpus.size() > 1) {
      ...
      if (Device::isGPUVendorAvailable(GPUVendor::NVIDIA)) {
        assertEqual(static_cast<int>(first_vendor), static_cast<int>(GPUVendor::NVIDIA),
  ...
  void test() override {
    ...
    for (size_t size : test_sizes) {
      ...
      for (size_t i = 0; i < 10 && i < size; ++i) {
        assertNear(arr.data()[i], 42.0, 1e-10, "Fill operation should work with GPU");
      }
      ...
      if (size >= 2) {
        assertNear(copy_arr.data()[0], 42.0, 1e-10, "Copy should preserve values");
        assertNear(copy_arr.data()[1], 42.0, 1e-10, "Copy should preserve values");
      }
    }
...
protected:
  void test() override {
    ...
    try {
      ...
      NDArray result = a + b;
      assertNear(result.data()[0], 3.0, 1e-10, "Addition should work even with fallback");
      
      result = a.matmul(b);
      assertTrue(result.shape()[0] == 2, "Matrix multiplication should work with fallback");
      
    } catch (const std::exception& e) {
      // GPU operations should not throw unhandled exceptions
      assertFalse(true, std::string("GPU operations should handle errors gracefully: ") + e.what());
    }
...
protected:
  void test() override {
    ...
    assertTrue(cpu_str == std::string("CPU"), "CPU device string should be correct");
    assertTrue(gpu_str == std::string("GPU"), "GPU device string should be correct");
    assertTrue(auto_str == std::string("AUTO"), "AUTO device string should be correct");
