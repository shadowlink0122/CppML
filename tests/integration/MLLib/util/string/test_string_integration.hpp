...
public:
    ModelConfigurationStringIntegrationTest() : TestCase("ModelConfigurationStringIntegrationTest") {}
...
protected:
    void test() override {
        ...
        assertEqual(size_t(2), output.size(), "String-configured model should have correct output size");
        
        for (double val : output) {
            assertTrue(!std::isnan(val) && !std::isinf(val), "String-configured model should produce valid outputs");
        }
    }
};
...
public:
    ErrorMessageFormattingIntegrationTest() : TestCase("ErrorMessageFormattingIntegrationTest") {}
...
protected:
    void test() override {
        ...
        assertEqual(size_t(3), output.size(), "Error handling should preserve correct functionality");
        ...
        assertTrue(full_message.find("Model") != std::string::npos, "Error messages should contain context");
        assertTrue(full_message.find("input_size") != std::string::npos, "Error messages should contain details");
        ...
        assertTrue(error_recovery_works, "Error handling should allow graceful recovery");
    }
};
...
public:
    DataFormatConversionIntegrationTest() : TestCase("DataFormatConversionIntegrationTest") {}
...
protected:
    void test() override {
        ...
        std::string csv_data = "0.1,0.2,0.3,1.0\n0.4,0.5,0.6,0.0\n0.7,0.8,0.9,1.0\n";
        ...
        assertEqual(size_t(3), X.size(), "Should parse correct number of samples from string");
        assertEqual(size_t(3), Y.size(), "Should parse correct number of labels from string");
        ...
        assertTrue(prediction_string.find("predictions") != std::string::npos, "Should format predictions as string");
        assertTrue(prediction_string.find("sample_") != std::string::npos, "Should include sample identifiers");
    }
};
...
public:
    StringParameterHandlingIntegrationTest() : TestCase("StringParameterHandlingIntegrationTest") {}
...
protected:
    void test() override {
        ...
        assertTrue(X.size() == static_cast<size_t>(batch_size), "Training data should match parsed batch size");
        ...
        assertTrue(training_successful, "String parameter parsing should enable successful training");
        ...
        assertTrue(model_name.find("neural_network") != std::string::npos, "Model name should contain descriptive terms");
        assertTrue(model_type == "sequential", "Model type should be correctly identified");
