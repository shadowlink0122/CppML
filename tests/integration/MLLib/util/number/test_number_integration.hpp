...
public:
    NumericalStabilityIntegrationTest() : TestCase("NumericalStabilityIntegrationTest") {}
...
protected:
    void test() override {
        ...
        std::vector<std::vector<double>> Y = {
            {1.0, 0.0}, {0.0, 1.0}, {0.5, 0.5}, {0.8, 0.2}, {0.3, 0.7}
        };
        ...
        assertNoThrow([&]() {
            model->train(X, Y, loss, optimizer,
                [&](int epoch, double current_loss) {
                    if (std::isnan(current_loss) || std::isinf(current_loss) || current_loss < 0) {
        ...
        assertTrue(numerically_stable, "Training should maintain numerical stability");
        ...
        for (const auto& input : X) {
            ...
            assertEqual(size_t(2), pred.size(), "Challenging inputs should produce correct output size");
            
            for (double val : pred) {
                assertTrue(!std::isnan(val) && !std::isinf(val), "Predictions should be numerically valid");
                assertTrue(val >= 0.0 && val <= 1.0, "Sigmoid outputs should be in [0,1]");
                assertTrue(val > 1e-15 && val < 1.0 - 1e-15, "Predictions should avoid extreme values");
            }
        }
...
public:
    PrecisionHandlingIntegrationTest() : TestCase("PrecisionHandlingIntegrationTest") {}
...
protected:
    void test() override {
        ...
        for (size_t i = 0; i < X.size(); i++) {
            ...
            assertEqual(size_t(1), pred.size(), "High precision input should produce scalar output");
            
            // Prediction should be reasonable (not exactly target, but not too far)
            double prediction_error = std::abs(pred[0] - Y[i][0]);
            assertTrue(prediction_error < 1.0, "High precision prediction should be reasonable");
            assertTrue(!std::isnan(pred[0]) && !std::isinf(pred[0]), "High precision prediction should be valid");
        }
        ...
        assertTrue(precision_diff < 1e-14, "Identical inputs should produce identical outputs");
    }
};
...
protected:
    void test() override {
        ...
        std::vector<std::vector<double>> Y = {
            {0.0, 1.0}, {1.0, 0.0}, {0.5, 0.5}, {0.1, 0.9}, {0.7, 0.3}
        };
        ...
        for (const auto& input : edge_test_inputs) {
            std::vector<double> pred = model->predict(input);
            assertEqual(size_t(2), pred.size(), "Edge case input should produce correct output size");
            
            for (double val : pred) {
                assertTrue(!std::isnan(val) && !std::isinf(val), "Edge case prediction should be valid");
                assertTrue(val >= 0.0 && val <= 1.0, "Edge case sigmoid output should be in [0,1]");
            }
        }
        ...
        for (double val : pred_zero) {
            assertTrue(!std::isnan(val) && !std::isinf(val), "Near-zero prediction should be valid");
        }
        for (double val : pred_one) {
            assertTrue(!std::isnan(val) && !std::isinf(val), "Near-one prediction should be valid");
        }
    }
};
...
public:
    MathematicalOperationsIntegrationTest() : TestCase("MathematicalOperationsIntegrationTest") {}
...
protected:
    void test() override {
        ...
        model->add(std::make_shared<activation::Sigmoid>()); // Exponential functions
        ...
        for (const auto& input : math_test_inputs) {
            std::vector<double> pred = model->predict(input);
            assertEqual(size_t(2), pred.size(), "Mathematical input should produce correct output size");
            
            for (double val : pred) {
                assertTrue(!std::isnan(val) && !std::isinf(val), "Mathematical prediction should be valid");
                assertTrue(val >= 0.0 && val <= 1.0, "Mathematical sigmoid output should be in [0,1]");
                
                // Check that mathematical operations preserve precision
                assertTrue(std::abs(val - std::round(val * 1e6) / 1e6) < 1e-6, 
