  ...
  void test() override {
    ...
    for (double lr : learning_rates) {
      ...
      assertEqual(size_t(1), pred.size(),
                  "Prediction should have correct size");
...
// Check if Adam optimizer is available
    ...
    assertEqual(size_t(2), pred1.size(),
                "Prediction 1 should have correct size");
    assertEqual(size_t(2), pred2.size(),
                "Prediction 2 should have correct size");

    for (double val : pred1) {
      assertTrue(!std::isnan(val) && !std::isinf(val),
                 "Prediction 1 values should be valid");
    }
    for (double val : pred2) {
      assertTrue(!std::isnan(val) && !std::isinf(val),
                 "Prediction 2 values should be valid");
    }
  }
};
...
public:
  OptimizerComparisonIntegrationTest()
      : TestCase("OptimizerComparisonIntegrationTest") {}
...
protected:
  void test() override {
    ...
    std::vector<std::vector<double>> X = {{0.1, 0.2},
                                          ...
                                          {0.7, 0.8},
                                          {0.9, 0.8}};
    std::vector<std::vector<double>> Y = {{0.8}, {0.6}, {0.4}, {0.2}, {0.1}};
    ...
    // Test SGD
    {
      ...
      assertEqual(size_t(1), sgd_pred.size(),
                  "SGD prediction should have correct size");
    ...
    // Test Adam
    {
      ...
      assertEqual(size_t(1), adam_pred.size(),
