    ...
    void test() override {
        ...
        try {
            ...
            if (Device::isGPUAvailable()) {
                assertTrue(actual_device == DeviceType::GPU, "Model should use GPU when available");
            } else {
                assertTrue(actual_device == DeviceType::CPU, "Model should fallback to CPU when GPU unavailable");
            }
            ...
            assertTrue(gpu_model->get_device() == DeviceType::CPU, "Should switch to CPU");
            
            // Test that inference still works after device switch
            std::vector<double> test_after_switch = gpu_model->predict({0, 1});
            assertTrue(test_after_switch.size() == 1, "Output should have correct size after device switch");
            
        } catch (const std::exception& e) {
            assertFalse(true, std::string("GPU integration test failed: ") + e.what());
        }
...
public:
    GPUModelComplexityIntegrationTest() : TestCase("GPUModelComplexityIntegrationTest") {}
...
protected:
    void test() override {
        ...
        try {
            ...
            assertFalse(true, std::string("Complex GPU model test failed: ") + e.what());
        }
...
protected:
    void test() override {
        ...
        try {
            ...
            assertNear(result.data()[0], 4.0, 1e-10, "Array operations should work correctly");
            assertNear(result.data()[99], 4.0, 1e-10, "Array operations should work for all elements");
            ...
            assertTrue(mat_result.shape()[0] == 10, "Matrix result should have correct shape");
            assertTrue(mat_result.shape()[1] == 10, "Matrix result should have correct shape");
            
            // Check that result is reasonable (0.1 * 0.2 * 10 = 0.2 for each element)
            assertNear(mat_result.data()[0], 0.2, 0.01, "Matrix multiplication should produce expected results");
            
        } catch (const std::exception& e) {
            assertFalse(true, std::string("GPU memory integration test failed: ") + e.what());
        }
...
protected:
    void test() override {
        ...
        try {
            ...
            assertTrue(gpu_output.size() == 1, "GPU model output should have correct size");
            assertTrue(cpu_output.size() == 1, "CPU model output should have correct size");
            ...
            assertTrue(output1.size() == 2, "Output should maintain size through device switches");
            assertTrue(output2.size() == 2, "Output should maintain size through device switches");
            assertTrue(output3.size() == 2, "Output should maintain size through device switches");
            
            assertTrue(device2 == DeviceType::CPU, "Should switch to CPU");
            
            // If GPU is available, device3 should be GPU, otherwise CPU
            if (Device::isGPUAvailable()) {
                assertTrue(device3 == DeviceType::GPU, "Should switch back to GPU when available");
            } else {
                assertTrue(device3 == DeviceType::CPU, "Should stay on CPU when GPU not available");
            }
            
        } catch (const std::exception& e) {
            assertFalse(true, std::string("Cross-device integration test failed: ") + e.what());
        }
