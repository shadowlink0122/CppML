...
__global__ void add_kernel(const double* a, const double* b, double* result, size_t size) {
...
__global__ void subtract_kernel(const double* a, const double* b, double* result, size_t size) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < size) {
        result[idx] = a[idx] - b[idx];
    }
...
__global__ void multiply_kernel(const double* a, const double* b, double* result, size_t size) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < size) {
        result[idx] = a[idx] * b[idx];
    }
...
__global__ void add_scalar_kernel(const double* a, double scalar, double* result, size_t size) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < size) {
...
__global__ void multiply_scalar_kernel(const double* a, double scalar, double* result, size_t size) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < size) {
        result[idx] = a[idx] * scalar;
    }
...
void cuda_add(const double* h_a, const double* h_b, double* h_result, size_t size) {
    ...
    try {
        ...
        // Copy result back
        CUDA_CHECK(cudaMemcpy(h_result, d_result, byte_size, cudaMemcpyDeviceToHost));
...
void cuda_multiply_scalar(const double* h_a, double scalar, double* h_result, size_t size) {
    ...
    try {
        ...
        int grid_size = (size + BLOCK_SIZE - 1) / BLOCK_SIZE;
        multiply_scalar_kernel<<<grid_size, BLOCK_SIZE>>>(d_a, scalar, d_result, size);
        CUDA_CHECK(cudaGetLastError());
        CUDA_CHECK(cudaDeviceSynchronize());
