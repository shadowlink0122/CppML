...
std::unique_ptr<DenseAutoencoder> DenseAutoencoder::create_simple(
  int input_dim, int latent_dim, 
  double compression_ratio,
  DeviceType device) {
  
  // Calculate intermediate dimension
  int intermediate_dim = static_cast<int>(
    std::sqrt(input_dim * latent_dim * compression_ratio));
  
  std::vector<int> hidden_dims = {intermediate_dim};
  
  return std::make_unique<DenseAutoencoder>(
    input_dim, latent_dim, hidden_dims, device);
}

std::unique_ptr<DenseAutoencoder> DenseAutoencoder::create_deep(
  int input_dim, int latent_dim,
  int num_layers,
  DeviceType device) {
  ...
  for (int i = 1; i <= num_layers; ++i) {
    ...
    int dim = static_cast<int>(
      input_dim * (1.0 - ratio) + latent_dim * ratio);
