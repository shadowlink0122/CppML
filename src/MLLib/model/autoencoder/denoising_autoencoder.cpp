...
void DenoisingAutoencoder::train(const std::vector<NDArray>& clean_data,
                                loss::BaseLoss& loss,
  ...
  BaseAutoencoder::train(clean_data, loss, optimizer, epochs, batch_size, validation_data, callback);
}
...
std::unique_ptr<DenoisingAutoencoder> DenoisingAutoencoder::create_for_images(
  ...
  std::vector<int> hidden_dims = {input_dim / 2, input_dim / 4}; // Progressive compression
  
  return std::make_unique<DenoisingAutoencoder>(
    input_dim, latent_dim, hidden_dims, noise_factor, NoiseType::GAUSSIAN, device);
}
...
NDArray DenoisingAutoencoder::add_noise(const NDArray& input) {
  ...
  switch (denoising_config_.noise_type) {
    case NoiseType::GAUSSIAN:
      return add_gaussian_noise(input);
    case NoiseType::SALT_PEPPER:
      return add_salt_pepper_noise(input);
    case NoiseType::DROPOUT:
      return add_dropout_noise(input);
    case NoiseType::UNIFORM:
      return add_uniform_noise(input);
    default:
      return input;
  }
}
...
double DenoisingAutoencoder::calculate_psnr(const NDArray& clean, const NDArray& noisy) {
  // Simplified PSNR calculation - placeholder
  return 20.0; // Default PSNR value
}

double DenoisingAutoencoder::calculate_ssim(const NDArray& clean, const NDArray& reconstructed) {
