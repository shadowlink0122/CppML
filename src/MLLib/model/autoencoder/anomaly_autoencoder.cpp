...
AnomalyDetector::AnomalyDetector(const AutoencoderConfig& config,
    ...
    : BaseAutoencoder(config), anomaly_config_(anomaly_config), threshold_(0.0),
      threshold_calculated_(false) {}

AnomalyDetector::AnomalyDetector(int input_dim, int latent_dim,
                                 const std::vector<int>& hidden_dims,
                                 double threshold_percentile, DeviceType device)
    : BaseAutoencoder(AutoencoderConfig()), threshold_(0.0),
      threshold_calculated_(false) {
...
void AnomalyDetector::train_on_normal(
    const std::vector<NDArray>& normal_data, loss::BaseLoss& loss,
  ...
  train(normal_data, loss, optimizer, epochs, batch_size, validation_data,
        callback);
}

void AnomalyDetector::calculate_threshold(
    const std::vector<NDArray>& normal_data) {
...
AnomalyResults
AnomalyDetector::detect_anomalies(const std::vector<NDArray>& test_data,
...
std::unique_ptr<AnomalyDetector> AnomalyDetector::create_for_sensors(
    int num_sensors, int latent_dim, double compression_ratio,
    double threshold_percentile, DeviceType device) {
  ...
  return std::make_unique<AnomalyDetector>(num_sensors, latent_dim, hidden_dims,
                                           threshold_percentile, device);
}

std::unique_ptr<AnomalyDetector> AnomalyDetector::create_for_timeseries(
    int window_size, int num_features, int latent_dim,
    double threshold_percentile, DeviceType device) {
  ...
  return std::make_unique<AnomalyDetector>(input_dim, latent_dim, hidden_dims,
                                           threshold_percentile, device);
}

void AnomalyDetector::save(const std::string& base_path, bool save_json,
                           bool save_binary) {
  BaseAutoencoder::save_legacy(base_path, save_json, save_binary);
...
bool AnomalyDetector::load(const std::string& base_path) {
  bool success = BaseAutoencoder::load_legacy(base_path);
...
double AnomalyDetector::calculate_percentile_threshold(
...
double
AnomalyDetector::calculate_std_threshold(const std::vector<double>& errors) {
  if (errors.empty()) return 0.0;
...
void AnomalyDetector::calculate_performance_metrics(
    AnomalyResults& results, const std::vector<bool>& ground_truth) {
  ...
  int total_predicted_positives =
      results.true_positives + results.false_positives;
  int total_samples = static_cast<int>(ground_truth.size());

  if (total_predicted_positives > 0) {
    results.precision =
        static_cast<double>(results.true_positives) / total_predicted_positives;
  }

  if (total_positives > 0) {
    results.recall =
        static_cast<double>(results.true_positives) / total_positives;
  }
  ...
  results.accuracy =
      static_cast<double>(results.true_positives + results.true_negatives) /
      total_samples;
}

void AnomalyDetector::update_adaptive_threshold(
    const std::vector<double>& recent_errors) {
  if (!anomaly_config_.adaptive_threshold || recent_errors.empty()) {
    return;
  }

  // Simple adaptive threshold update - in practice you'd use more sophisticated
  // methods
