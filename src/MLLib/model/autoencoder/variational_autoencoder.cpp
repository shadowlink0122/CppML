...
VariationalAutoencoder::VariationalAutoencoder(int input_dim, int latent_dim,
...
std::vector<NDArray> VariationalAutoencoder::interpolate(const NDArray& start_point,
...
void VariationalAutoencoder::train(const std::vector<NDArray>& training_data,
                                  loss::BaseLoss& loss,
...
double VariationalAutoencoder::get_current_kl_weight(int epoch) {
  if (vae_config_.use_kl_annealing) {
    double annealed_weight = vae_config_.kl_anneal_start + 
                            epoch * vae_config_.kl_anneal_rate;
...
std::unique_ptr<VariationalAutoencoder> VariationalAutoencoder::create_for_images(
  ...
  return std::make_unique<VariationalAutoencoder>(
    input_dim, latent_dim, hidden_dims, kl_weight, device);
}
...
std::unique_ptr<VariationalAutoencoder> VariationalAutoencoder::create_beta_vae(
  ...
  return std::make_unique<VariationalAutoencoder>(
    input_dim, latent_dim, hidden_dims, beta, device);
}
...
NDArray VariationalAutoencoder::reparameterize_sample(const NDArray& mean, const NDArray& log_var) {
...
double VariationalAutoencoder::calculate_kl_loss(const NDArray& mean, const NDArray& log_var) {
...
NDArray VariationalAutoencoder::sample_standard_normal(const std::vector<int>& shape) {
